...
export async function waitForChangeSet(
  ...
  const ret = await waitFor(async () => {
    ...
    // eslint-disable-next-line max-len
    throw new Error(
      `Failed to create ChangeSet ${changeSetName} on ${stackName}: ${
        description.Status || 'NO_STATUS'
 ...
 */
export function changeSetHasNoChanges(description: DescribeChangeSetOutput) {
...
export class ParameterValues {
  ...
  constructor(
    ...
    if (missingRequired.length > 0) {
      throw new Error(
        `The following CloudFormation Parameters are missing a value: ${missingRequired.join(
          ', ',
        )}`,
      )
    }
  ...
  public hasChanges(currentValues: Record<string, string>): boolean {
    ...
    if (
      Object.entries(currentValues).some(
        ([key, value]) => !(key in this.values) || value !== this.values[key],
      )
    ) {
      return true
    }
